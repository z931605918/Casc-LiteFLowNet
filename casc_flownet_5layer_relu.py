...
class Matching(torch.nn.Module):
    def __init__(self, intLevel):
        ...
        self.fltBackwarp = [0.0, 20.0, 10.0, 5.0, 2.5, 1.25, 0.625][intLevel]
        ...
        if intLevel == 5:
            self.netUpflow = torch.nn.Sequential()
        ...
        self.netMain = torch.nn.Sequential(
            ...
            torch.nn.LeakyReLU(inplace=False, negative_slope=0.1),
            torch.nn.Conv2d(in_channels=32, out_channels=2, kernel_size=[0, 7, 7, 5, 5, 3, 3][intLevel], stride=1,
                            padding=[0, 3, 3, 2, 2, 1, 1][intLevel])
        )
...
class Subpixel(torch.nn.Module):
    def __init__(self, intLevel):
        ...
        self.fltBackward = [0.0,  20.0 , 10.0, 5.0, 2.5, 1.25, 0.625][intLevel]
        ...
        self.netMain = torch.nn.Sequential(
            torch.nn.Conv2d(in_channels=[0, 130, 130, 130, 194, 258, 386][intLevel], out_channels=128, kernel_size=3,
                            stride=1, padding=1),
            torch.nn.LeakyReLU(inplace=False, negative_slope=0.1),
            torch.nn.Conv2d(in_channels=128, out_channels=64, kernel_size=3, stride=1, padding=1),
            torch.nn.LeakyReLU(inplace=False, negative_slope=0.1),
...
class Regularization(torch.nn.Module):
    def __init__(self, intLevel):
        ...
        self.intUnfold = [0, 7, 7, 5, 5, 3, 3][intLevel]
        ...
        if intLevel >= 5:
            ...
            self.netFeat = torch.nn.Sequential(
                torch.nn.Conv2d(in_channels=[0, 32, 32, 64, 96, 128, 192][intLevel], out_channels=128, kernel_size=1,
                                stride=1, padding=0),
                torch.nn.LeakyReLU(inplace=False, negative_slope=0.1)
            )
        ...
        self.netMain = torch.nn.Sequential(
            torch.nn.Conv2d(in_channels=[0, 131, 131, 131, 131, 131, 195][intLevel], out_channels=128, kernel_size=3,
                            stride=1, padding=1),
            torch.nn.LeakyReLU(inplace=False, negative_slope=0.1),
        ...
        if intLevel >= 5:
            self.netDist = torch.nn.Sequential(
                torch.nn.Conv2d(in_channels=32, out_channels=[0, 49, 49, 25, 25, 9, 9][intLevel],
                                kernel_size=[0, 7, 7, 5, 5, 3, 3][intLevel], stride=1,
                                padding=[0, 3, 3, 2, 2, 1, 1][intLevel])
            )
            ...
            self.netDist = torch.nn.Sequential(
                torch.nn.Conv2d(in_channels=32, out_channels=[0, 49, 49, 25, 25, 9, 9][intLevel],
                                kernel_size=([0, 7, 7, 5, 5, 3, 3][intLevel], 1), stride=1,
                                padding=([0, 3, 3, 2, 2, 1, 1][intLevel], 0)),
                torch.nn.Conv2d(in_channels=[0, 49, 49, 25, 25, 9, 9][intLevel],
                                out_channels=[0, 49, 49, 25, 25, 9, 9][intLevel],
                                kernel_size=(1, [0, 7, 7, 5, 5, 3, 3][intLevel]), stride=1,
                                padding=(0, [0, 3, 3, 2, 2, 1, 1][intLevel]))
            )

        # end

        self.netScaleX = torch.nn.Conv2d(in_channels=[0, 49, 49, 25, 25, 9, 9][intLevel], out_channels=1, kernel_size=1,
                                         stride=1, padding=0)
        self.netScaleY = torch.nn.Conv2d(in_channels=[0, 49, 49, 25, 25, 9, 9][intLevel], out_channels=1, kernel_size=1,
                                         stride=1, padding=0)
    ...
    def forward(self, tenFirst, tenSecond, tenFeaturesFirst, tenFeaturesSecond, tenFlow):
        tenDifference = (tenFirst - backwarp(tenInput=tenSecond, tenFlow=tenFlow * self.fltBackward)).pow(2.0).sum(1,
                                                                                                                   True).sqrt().detach() #返回u和v的difference的和
...
class en(torch.nn.Module):
    def __init__(self):
        ...
        self.Match_1=Matching(1)
        self.Subp=Subpixel(1)
        self.ReLu=Regularization(1)
    def forward(self, tenFirst, tenSecond, tenFeaturesFirst, tenFeaturesSecond, tenFlow):
        tenFlow=self.Match_1(tenFirst,tenSecond,tenFeaturesFirst,tenFeaturesSecond,tenFlow)
        tenFlow=self.Subp(tenFirst,tenSecond,tenFeaturesFirst,tenFeaturesSecond,tenFlow)
        tenFlow=self.ReLu(tenFirst,tenSecond,tenFeaturesFirst,tenFeaturesSecond,tenFlow)
        return tenFlow
...
class Network(torch.nn.Module):
    def __init__(self):
        ...
        self.pre_ReLU = Regularization(5)
        # end
        self.en=en()
    ...
    def forward(self, x ,flow):
        ...
        tenFeaturesFirst = self.netFeatures(tenFirst)    #tenFeaturesFirst[F1,F2,F3,F4]
        ...
        self.tenflo=torch.nn.functional.interpolate(input=flow,size=(int(tenFeaturesFirst[4].shape[2]),int(tenFeaturesSecond[4].shape[3])),mode='bilinear',align_corners=False)

        tenFlow = self.pre_ReLU(tenFirst[4], tenSecond[4], tenFeaturesFirst[4], tenFeaturesSecond[4], self.tenflo)
        tenFlows = []
        ...
        for intLevel in [ -1, -2, -3,-4  ]:
            ...
            tenFlows += [tenFlow]
        # end
        tenFlow=torch.nn.functional.interpolate(input=tenFlow,size=(tenFirst[0].shape[2],tenFirst[0].shape[3]),mode='bilinear',align_corners=False)
        tenFlows+=[tenFlow]
...
def casc_liteflownet(path=None):
    model = Network()
    if path is not None:
