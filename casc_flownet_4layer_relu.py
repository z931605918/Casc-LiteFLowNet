...
class en(torch.nn.Module):
    def __init__(self):
        ...
        self.upconv=torch.nn.ConvTranspose2d(in_channels=2,out_channels=2,kernel_size=4,stride=2,padding=1,bias=False,groups=2)
    def forward(self,tensor):
        out=self.upconv(tensor)
        return out
##########################################################
class Features(torch.nn.Module):
    ...
    def forward(self, tenInput):
        ...
        return [tenOne, tenTwo, tenThr, tenFou]
...
class Network(torch.nn.Module):
    def __init__(self):
        ...
        # end
        self.en=en()
        self.pre_ReLU = Regularization(4)
        self.netFeatures = Features()
        self.netMatching = torch.nn.ModuleList([ Matching(intLevel) for intLevel in [ 2, 3, 4  ] ])
        self.netSubpixel = torch.nn.ModuleList([ Subpixel(intLevel) for intLevel in [ 2, 3, 4  ] ])
        self.netRegularization = torch.nn.ModuleList([ Regularization(intLevel) for intLevel in [ 2, 3, 4  ] ])
    ...
    def forward(self, x ,flow,plot_inter=False):
        ...
        flow=torch.nn.functional.interpolate(input=flow,size=(int(tenFeaturesFirst[3].shape[2]),int(tenFeaturesSecond[3].shape[3])),mode='bilinear',align_corners=False)
        self.tenflo = self.pre_ReLU(tenFirst[3], tenSecond[3], tenFeaturesFirst[3], tenFeaturesSecond[3],flow)
        #self.tenflo=torch.nn.functional.interpolate(input=self.tenflo,size=(int(tenFeaturesFirst[3].shape[2]),int(tenFeaturesSecond[3].shape[3])),mode='bilinear',align_corners=False)
        tenFlow = self.tenflo
